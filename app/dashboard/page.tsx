...
export default async function DashboardPage() {
    ...
    const totalValue = allProducts.reduce((sum: any, product: any) => sum + Number(product.price) * Number(product.quantity), 0)

    const inStockCount = allProducts.filter((product: any) => Number(product.quantity) > 5).length
    const lowStockCount = allProducts.filter((product: any) => Number(product.quantity) <= 5 && Number(product.quantity) >= 1).length
    const outOfStockCount = allProducts.filter((product: any) => Number(product.quantity) === 0).length
            ...
            <main className="ml-64 p-8">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
                    <div className="bg-white rounded-lg border border-gray-200 p-6">
                        <h2 className="text-lg font-semibold text-gray-800 mb-6">Key Metrics</h2>
                        <div className="grid grid-cols-3 gap-6">
                            ...
                            <div className="text-center">
                                <div className="text-3xl font-bold text-gray-800">${Number(totalValue).toFixed(0)}</div>
                            ...
                            <div className="text-center">
                                <div className="text-3xl font-bold text-gray-800">{lowStock}</div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
                    {/* Stock Levels */}
                    <div className="bg-white rounded-lg border border-gray-200 p-6">
                        <h2 className="text-lg font-semibold text-gray-800 mb-5">Stock Levels</h2>
                        <div className="space-y-3">
                            {recent.map((product: any, key: any) => {
                    ...
                    <div className="bg-white rounded-lg border border-gray-200 p-6">
                        <div className="flex items-center justify-between mb-6">
                            <h2 className="text-lg font-semibold text-gray-800">Efficency</h2>
                        </div>
                        <div className="flex items-center justify-center">
                            <div className="relative w-48 h-48">
                                <div className="absolute inset-0 rounded-full border-8 border-gray-200"></div>
                                <div
                                    className="absolute inset-0 rounded-full border-8 border-cyan-600"
                                    style={{
                                        clipPath:
